...
def assignmentTopo():
    ...
    # =>fill in here command to add two queues to port 2 of the switch: Q0 with min rate 20000000 and max rate -SELECT THE PROPER RATE-, Q1 with min rate 50000000 and max rate -SELECT THE PROPER RATE-
    os.system('sudo ovs-vsctl set port s1-eth3 qos=@newqos -- --id=@newqos create qos type=linux-htb queues=0=@q0 -- --id=@q0 create queue other-config:max-rate=30000000')
    # =>fill in here command to add one queue to port 3 of the switch: Q0 with min rate 20000000 and max rate -SELECT THE PROPER RATE-
    os.system('sudo ovs-vsctl set port s1-eth4 qos=@newqos -- --id=@newqos create qos type=linux-htb queues=0=@q0,1=@q1 -- --id=@q0 create queue other-config:max-rate=1000000000000 --  --id=@q1 create queue other-config:max-rate=200000000')
    # =>fill in here command to add two queues to port 4 of the switch: you decide number of queues and rates
